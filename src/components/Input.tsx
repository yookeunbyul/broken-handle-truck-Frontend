...
export default function Input({
    ...
    children,
    ...props
}: InputProps & Omit<React.ComponentPropsWithoutRef<'input'>, keyof InputProps | 'className'>) {
    return (
        <div className="mx-auto w-[calc(100%-100px)] sm:w-[calc(100%-200px)] flex flex-col gap-1">
            <label htmlFor={id} className="mb-2 text-xs text-white/50 mix-blend-difference tracking-tight">
                {label}
            </label>
