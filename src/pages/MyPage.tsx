	...
	const handleEditNickname = () => {
		...
		if (isEditMode) {
			...
			if (nickname.trim() === '') {
				...
				toast.error('닉네임은 13자 이하로 입력해주세요.');
				return;
			}
						...
						<div className='flex justify-center'>
							<div className='rounded-full w-24 h-24 bg-review'>
								{!userInfo?.thumnail ? (
									...
									<img src={userInfo?.thumnail} className='rounded-full w-24 h-24 border-1 border-category' />
								)}
							</div>
						</div>
						<div className='flex-1 flex items-center gap-1 relative'>
							<div className='flex flex-col gap-y-1'>
								<div className='flex items-center gap-2.5'>
									<span
										className={`tracking-tighter text-lg pl-1 whitespace-nowrap ${isEditMode ? 'hidden' : 'inline'}`}
									>
										{nickname}
									</span>
									<input
										ref={nicknameRef}
										className={`outline-none bg-transparent whitespace-nowrap tracking-tighter text-lg pl-1 ${
											!isEditMode ? 'hidden' : 'inline'
										}`}
										style={{ maxWidth: `${nickname.length * 16 < 206 ? nickname.length * 16 : 205}px` }}
										type='text'
										value={nickname}
									...
									<button className='relative' onClick={handleEditNickname}>
										{isEditMode ? (
											...
											<EditIcon className='duration-300 stroke-gray hover:stroke-primary' width={20} height={20} />
										)}
									</button>
								</div>
