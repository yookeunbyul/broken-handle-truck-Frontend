...
const BottomNavBar = ({ isOpenCategory, setOpenCategory }: BottomNavBarProps) => {
    ...
    return (
        <div className="flex bg-black text-xs sticky bottom-0 z-[9999] bottom-navbar">
            <div
                className={[
            ...
            <div
                className={[
                    'flex flex-col flex-1 py-4 gap-1 items-center justify-center cursor-pointer',
                    isFocused('/notification') ? 'text-primary stroke-primary' : 'text-white stroke-white',
                ].join(' ')}
                onClick={() => handleClick('/notification')}
            >
                <div className="relative">
                    <BellIcon width={24} height={24} />
                    {isNewNotification && <div className="size-2 bg-primary rounded-full absolute -top-1 -right-1" />}
                </div>
                <p>알림</p>
            </div>
            <div
                className={[
                    'flex flex-col flex-1 py-4 gap-1 items-center justify-center cursor-pointer',
                    isFocused('/map') ? 'text-primary stroke-primary' : 'text-white stroke-white',
                ].join(' ')}
            ...
            <div
                className={[
                    'flex flex-col flex-1 py-4 gap-1 items-center justify-center cursor-pointer',
                    isFocused('/bookmark') ? 'text-primary stroke-primary' : 'text-white stroke-white',
                ].join(' ')}
            ...
            <div
                className={[
                    'flex flex-col flex-1 py-4 gap-1 items-center justify-center cursor-pointer',
                    isFocused('/my-page') ? 'text-primary stroke-primary' : 'text-white stroke-white',
                ].join(' ')}
